...
function simpleMaximum(node, parentCount) {
  const value = constraintValue(node, "max", "parent") ?? constraintValue(node, "max");
  return value === null ? Infinity : Math.max(0, value * Math.max(1, parentCount));
}
...
function allocateAdditional(group, amount, parentCount, selections, unitDefinition, index) {
  ...
  for (const childGroup of defaultOrderedChildGroups(group, childGroups)) {
    ...
    const requested = Math.min(remaining, capacity);
    if (requested <= 0) continue;
    remaining -= allocateAdditional(childGroup, requested, parentCount, selections, unitDefinition, index);
  }
    ...
    if (current < desired) {
      ...
      const ordered = hasExplicit
        ...
        : [...entryChildren].sort((a, b) =>
          ...
          - dynamicMaximum(a, parentCount, selections, unitDefinition, index)
        );
      for (const candidate of ordered) {
        if (remaining <= 0) break;
        const currentCount = Number(selections[candidate.id] || 0);
        const available = dynamicMaximum(candidate, parentCount, selections, unitDefinition, index) - currentCount;
        const add = Math.min(remaining, available);
...
function createDefaultRosterEntry(unitDefinition, instanceId = `${unitDefinition.id}-1`) {
  ...
  let repaired = repairDefaultLoadout(unitDefinition, entry);
  ...
  if (unitDefinition?.name === "Deathshroud Terminators") {
    ...
    if (championGauntlet) {
      const maximum = evaluatedLimits(championGauntlet, repaired, index, unitDefinition).maximum;
      if (maximum >= 2) repaired = setSelection(unitDefinition, repaired, championGauntlet.id, 2, false);
    }
  }
...
function setUnitSize(unitDefinition, entry, requestedSize) {
  ...
  if (remaining > 0) {
    for (const record of candidates) {
      const capacity = record.maximum - record.current;
    ...
    for (const record of candidates) {
      const capacity = record.current - record.minimum;
      const change = Math.min(-remaining, capacity);
      if (change <= 0) continue;
      next = unitDefinition.name === "Sword Brethren Squad"
        ? setSwordBrethrenCountPreservingWeapons(unitDefinition, next, record.node, record.current - change, index)
...
function listSelectableOptions(unitDefinition) {
  const index = buildTreeIndex(unitDefinition);
  return index.all
    .filter(node => !["unit", "group"].includes(node.kind) && !node.hidden)
    .map(node => ({
      id: node.id,
      definitionId: node.definitionId,
      name: node.name,
